...
# from components.episode_buffer_graph import ReplayBuffer
...
def run_sequential(args, logger):
    ...
    if args.EA:
        pop = []
        if args.SameController:
            if args.LTSCG:
                mac = mac_REGISTRY["EA_" + args.mac + "_ltscg"](buffer.scheme, groups, args)
            elif args.DICG:
                mac = mac_REGISTRY["EA_" + args.mac + "_dicg"](buffer.scheme, groups, args)
            else:
                mac = mac_REGISTRY["EA_" + args.mac](buffer.scheme, groups, args)
            for i in range(5):
                    if args.LTSCG:
                        pop.append(mac_REGISTRY["EA_" + args.mac + "_ltscg"](buffer.scheme, groups, args))
                    elif args.DICG:
                        pop.append(mac_REGISTRY["EA_" + args.mac + "_dicg"](buffer.scheme, groups, args))
                    else:
                        pop.append(mac_REGISTRY["EA_" + args.mac](buffer.scheme, groups, args))
        else:
        ...
        if getattr(args, "runner_scope", "episodic") == "episodic":
            ...
            for i in range(repeat_num):
                ...
                if buffer.can_sample(args.batch_size) and (
                    ...
                    # 这里是拿到部分的episode信息，但这才是大部分框架流程的选择。
                    ...
                    if args.EA:
                        ...
                        if args.LTSCG:
                            learner.train(episode_sample, all_teams, max_ep_t, runner.t_env, episode)
                        else:
                            learner.train(episode_sample[:, :max_ep_t], all_teams, max_ep_t, runner.t_env,
                                          episode)
                    else:
                        if args.LTSCG:
                            learner.train(episode_sample, max_ep_t, runner.t_env, episode)
                        else:
                            learner.train(episode_sample[:, :max_ep_t], max_ep_t, runner.t_env, episode)
